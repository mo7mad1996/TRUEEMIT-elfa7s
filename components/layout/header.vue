...
<template>
	<header class="pt-1 sticky top-0 left-0 z-50 backdrop-blur-sm mx-auto container">
		...
		>
			...
			<div class="flex flex-1 justify-end relative" v-click-outside="() => (menu = false)">
				<button
					class="p-2 flex bg-slate-100 hover:bg-slate-200 aspect-square rounded"
					@click.prevent.stop="menu = !menu"
				>
					<font-awesome-icon icon="fa-solid fa-bars" class="pointer-events-none" />
				</button>

				<Transition>
					<ul
						v-if="menu"
						class="absolute top-full py-0.5 min-w-36 z-50 mt-2 rounded origin-top overflow-hidden flex flex-col gap-0 backdrop-blur-2xl shadow-2xl shadow-black/30 ring-1 ring-black/10"
					>
						<li
							class="group flex gap-px flex-col px-1 py-0.5 bg-white"
						...
						>
							<nuxt-link
								...
								:to="link.to"
							>
								<font-awesome-icon v-if="link.icon" :icon="link.icon" class="text-sm opacity-75" />
		...
		return {
			...
			links: [
				...
				},
				{
					...
					show: true,
					customClass: "!bg-red-500 px-6  group-hover:!bg-red-600 justify-center text-white shadow",
					action() {
						this.$auth.logout();
					},
				},
