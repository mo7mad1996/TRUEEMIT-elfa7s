  ...
  <section class="container grid grid-cols-2 gap-4">
    <div class="col-span-1">
      ...
      <FileDropAble
        ...
        @input="
          (image) =>
            (car.car_images = [...car?.car_images, { image, description: '' }])
        "
        @delete="
          (fileName) =>
            (car.car_images = car.car_images.filter(
      ...
      >
        <template v-slot="{ removeFile }">
          <ul v-if="car.images.length" class="file-list">
            <li
              v-for="(file, n) in car.car_images"
              :key="n"
              class="file-item simple_form"
            >
    ...
    <div class="col-span-1">
      ...
      <FileDropAble
        ...
        :id="car._id"
        @input="
          (image) => (car.images = [...car?.images, { image, description: '' }])
        "
        @delete="
          (fileName) =>
            (car.images = car.images.filter(({ image }) => image !== fileName))
        "
      >
        <template v-slot="{ removeFile }">
          <ul v-if="car.images.length" class="file-list">
            <li
              v-for="(file, n) in car.images"
              :key="(file, n)"
              class="file-item simple_form"
            >
              <img :src="file.image" />
